...
exports.command = 'notify-sns <event> <snsTopicArn> <s3Path>'
...
exports.builder = function (yargs:yargs.Argv) {
  yargs.demand(['event'])
    ...
    .describe('snsTopicArn', 'SNS topic ARN')
...
exports.handler = async function (argv) {
  try {
    ...
    const numObjects = await common.forEachS3KeyInPrefix(s3, s3Path.bucket, s3Path.key, async function (object:AWS.S3.Object) {
      ...
      if (argv.dryrun) {
        console.log(`(dryrun) Send ${argv.event} to ${argv.snsTopicArn} for ${object.Key}`)
      } else {
        console.log(`Send ${argv.event} to ${argv.snsTopicArn} for ${object.Key}`)
        await sender.sendSNSNotification(sns, argv.snsTopicArn, {
